...
const GenericMap: React.FC<GenericMapProps> = ({ dataType, fetchUrl, fetchByDateUrl, legendTitle }) => {
  ...
  const handleDateChange = async (date: string) => {
    ...
    const url = date === today ? fetchUrl : fetchByDateUrl; // Gunakan fetchUrl untuk hari ini
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col">
      ...
      >
        ...
        {boundaryData && !error && ( // Nonaktifkan GeoJSON saat ada error
          <GeoJSON
            data={boundaryData}
            ...
            onEachFeature={(feature, layer: L.GeoJSON) => {
              layer.on({
                mouseover: (e: L.LeafletMouseEvent) => {
                  if (error) return; // Jangan lakukan apa-apa jika ada error
                  layer.setStyle({
                    weight: 1.2,
                    color: "#1a4971",
                    fillOpacity: 0.1,
