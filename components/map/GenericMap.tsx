...
// // import * as GeoJSONTypes from "geojson";
// // import * as L from "leaflet";
// // import * as turf from "@turf/turf"; // Tambahkan turf untuk pengecekan batas
// // import { getBoundaryStyle } from "@/utils/map";
// // import { interpolateColor, interpolatePM25Color } from "@/utils/color";
...
// //                   const isInside = boundaryData.features.some((f: GeoJSONTypes.Feature) =>
// //                     turf.booleanPointInPolygon(point, f as GeoJSONTypes.Feature<GeoJSONTypes.Polygon | GeoJSONTypes.MultiPolygon>)
// //                   );
...
// //                   // Buat tooltip baru hanya jika dalam batas
// //                   if (mapRef.current) {
// //                     tooltipRef.current = L.tooltip({
// //                       sticky: true,
// //                       direction: "top",
...
//         const allZeroOrNull = data.features.every((f: GeoJSONTypes.Feature<GeoJSONTypes.Point, { aod_value?: number; pm25_value?: number }>) => {
//           const value = dataType === "aod" ? f.properties?.aod_value : f.properties?.pm25_value;
//           return value === null || value === undefined || value === 0;
//         });

//         if (allZeroOrNull) {
//           setGeoData({ ...data, features: [] }); // Set empty features untuk menandakan data kosong
//           throw new Error("Data pada hari ini kosong atau bernilai 0");
//         }

//         const validFeatures = data.features.filter((f: GeoJSONTypes.Feature<GeoJSONTypes.Point, { aod_value?: number; pm25_value?: number }>) => {
//           const value = dataType === "aod" ? f.properties?.aod_value : f.properties?.pm25_value;
//           return f.geometry && value !== null && value !== undefined && !isNaN(value) && value > 0;
//         });
...
//                 mouseover: (e: L.LeafletMouseEvent) => {
//                   const point = L.latLng(e.latlng);
//                   const isInside = L.geoJSON(feature.geometry).getBounds().contains(point);
//                   if (!isInside) {
//                     clearTooltip();
//                     return;
//                   }
...
//                   // Cari fitur GeoJSON yang sesuai dengan wilayah
//                   const dataFeature = geoData?.features.find((f: GeoJSONTypes.Feature) => {
//                     if (!f.geometry) return false;
//                     try {
//                       const centroid = turf.centroid(f.geometry);
//                       const featurePoint = turf.point(centroid.geometry.coordinates);
//                       const distance = turf.distance(turf.point([e.latlng.lng, e.latlng.lat]), featurePoint, { units: "degrees" });
//                       return distance < 0.002; // Toleransi jarak untuk mencocokkan
//                     } catch {
//                       return false;
//                     }
//                   });
...
//         // Filter fitur yang memiliki nilai valid
//         const validFeatures = data.features.filter((f: GeoJSONTypes.Feature<GeoJSONTypes.Point | GeoJSONTypes.Polygon | GeoJSONTypes.MultiPolygon, { aod_value?: number; pm25_value?: number }>) => {
//           const value = dataType === "aod" ? f.properties?.aod_value : f.properties?.pm25_value;
//           return f.geometry && value !== null && value !== undefined && !isNaN(value) && value > 0;
//         });
...
//         setGeoData({ ...data, features: validFeatures });
//         if (validFeatures.length === 0) {
//           setError("Tidak ada data valid untuk tanggal ini");
//         }
//       } catch (err) {
//         setGeoData(null);
//         setError(err instanceof Error ? err.message : "Terjadi kesalahan saat memuat data");
...
//                   // Cari data asli dari geoData untuk wilayah ini
//                   const dataFeature = geoData?.features.find((f: GeoJSONTypes.Feature) => {
//                     if (!f.geometry) return false;
