...
// ): geometry is GeoJSONTypes.Polygon | GeoJSONTypes.MultiPolygon => {
...
//     // Tentukan satuan untuk PM2.5, kosong untuk AOD atau "No data"
...
//   // Type guard untuk memastikan geometri adalah Polygon atau MultiPolygon di handleMouseMove
...
const GenericHeatMapLayer: React.FC<HeatMapLayerProps> = ({
  ...
  const handleMouseMove = useCallback(
    debounce((e: L.LeafletMouseEvent) => {
      if (!map || !boundaryData || !spatialIndexRef.current || error) { // Periksa error
        if (tooltipRef.current) {
          tooltipRef.current.remove();
          tooltipRef.current = null;
        }
  ...
  useEffect(() => {
    console.log("DEBUG: useEffect for ImageOverlay - isLoading:", isLoading, "geoData:", !!geoData, "boundaryData:", !!boundaryData);
    if (!map || !geoData || !boundaryData || isLoading || error) { // Tambahkan pengecekan error
      if (staticLayerRef.current) {
        map.removeLayer(staticLayerRef.current);
        staticLayerRef.current = null;
